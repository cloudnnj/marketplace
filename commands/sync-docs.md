...
Automatically update project documentation with the current state of the codebase. This command runs non-interactively and updates README.md, CLAUDE.md, and docs/roadmap/technical-roadmap.md.
...
4. **Remove Human-Oriented Content**
   ...
   - Roadmap details belong in docs/roadmap/technical-roadmap.md
...
Update `docs/roadmap/technical-roadmap.md`:
...
3. **Stage and commit**
   ```bash
   git add README.md CLAUDE.md docs/roadmap/technical-roadmap.md
...
4. **Push and create PR**
   ...
   Then use MCP tools:
   - **Check for existing PR**: Use `mcp__github__list_pull_requests` with `owner`, `repo`, `head: "<owner>:chore/doc-update"`, `state: "open"`.
   - **If PR exists**: The push already updated it. No further action needed.
   - **If no PR exists**: Use `mcp__github__create_pull_request` with:
     - `owner`, `repo`
     - `title: "chore(docs): automated documentation sync"`
     - `head: "chore/doc-update"`
     - `base: "main"`
     - `body`: Include sections for Files Updated, CLAUDE.md Optimization, and Review Checklist.
